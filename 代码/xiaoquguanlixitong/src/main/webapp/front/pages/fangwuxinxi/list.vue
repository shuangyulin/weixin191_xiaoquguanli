<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input v-model="searchForm.fangwuxinxiName" type="text" placeholder="房屋名称"></input>
		</view>
	...
	<scroll-view scroll-x="true" class="tabView"
		...
		<view :style='categoryName===item.codeIndex?sactiveItem:sitem' class="tab"
			  v-for="(item,index) in fangwuxinxiTypesList" :key="index"
...
<view class="list" :style='{"borderRadius":0,"backgroundColor":"rgba(255, 255, 255, 0)"}'>
    <view style="margin: 10rpx 10rpx 10rpx 10rpx;"
        ...
        <image :style='{"borderRadius":"40rpx","width":"50%","height":"200rpx"}' mode="aspectFill" class="listmpic"
               :src="product.fangwuxinxiPhoto?product.fangwuxinxiPhoto.split(',')[0]:''"></image>
        <view class="listmr">
            <view class="col3 f30 elip mb15"
                  :style='{"fontSize":"28rpx","lineHeight":"56rpx","color":"rgba(255, 255, 255, 1)","textAlign":"center"}'>
                {{product.fangwuxinxiName}}
            </view>
        </view>
           <view style="display: flex;justify-content: space-between;float: left;margin-top: 20%;">
            <text style="margin-left: 50rpx;" v-if="isAuth('fangwuxinxi','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">
                修改
            </text>
            <text v-if="isAuth('fangwuxinxi','删除')" class="cuIcon-delete"
            ...
            v-if="isAuth('fangwuxinxi','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
	   		...
	   		//当前表的 房屋类型 字段 字典表查询方法
		  	let fangwuxinxiTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'fangwuxinxi_types'
		  	});
		  	this.fangwuxinxiTypesList = fangwuxinxiTypes.data.list;
		...
		methods: {
			//查询条件切换
			queryChange(e) {
				this.searchForm.fangwuxinxiName = "";
			},
			//类别搜索
				...
				let params = {
					...
					limit: mescroll.size
							...
							if (this.categoryName != 0) {
                                params.fangwuxinxiTypes = this.categoryName
                            }
			...
			async search() {
				...
				if (this.searchForm.fangwuxinxiName) {
                    searchForm['fangwuxinxiName'] = this.searchForm.fangwuxinxiName
                }
				if (this.searchForm.fangwuxinxiName) {
					searchForm['fangwuxinxiName'] = this.searchForm.fangwuxinxiName
				}
