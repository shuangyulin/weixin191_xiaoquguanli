	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
		...
		<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			<view class="search-form round">
				<input v-model="searchForm.ruzhudengjiName" type="text" placeholder="入住地址"></input>
			</view>
		...
		<scroll-view scroll-x="true" class="tabView"
			...
			<view :style='categoryName===item.codeIndex?sactiveItem:sitem' class="tab"
				v-for="(item,index) in ruzhudengjiTypesList" :key="index"
		...
		<view class="list" :style='{"borderRadius":0,"backgroundColor":"rgba(255, 255, 255, 0)"}'>
			<view style="margin: 10rpx 10rpx 10rpx 10rpx;" @tap="onDetailTap(product)" class="listm flex flex-between"
				:style='{"borderRadius":"40rpx","backgroundColor":"#FE8010"}' v-for="(product,index) in list"
				:key="index">
				<image :style='{"borderRadius":"40rpx","width":"50%","height":"200rpx"}' mode="aspectFill"
					class="listmpic" :src="product.ruzhudengjiPhoto?product.ruzhudengjiPhoto.split(',')[0]:''"></image>
				<view class="listmr">
					<view class="col3 f30 elip mb15"
						:style='{"fontSize":"28rpx","lineHeight":"56rpx","color":"rgba(255, 255, 255, 1)","textAlign":"center"}'>
						{{product.ruzhudengjiName}}
					</view>
				</view>
				<view style="display: flex;justify-content: space-between;float: left;margin-top: 20%;">
					<text style="margin-left: 50rpx;" v-if="isAuth('ruzhudengji','修改')" class="cuIcon-edit"
						@click.stop="onUpdateTap(product.id)">
						修改
					</text>
					<text v-if="isAuth('ruzhudengji','删除')" class="cuIcon-delete"
			...
			v-if="isAuth('ruzhudengji','新增')" class="add-btn" @click="onAddTap()">新增</button>
...
<script>
	export default {
		...
		async onShow() {
			...
			//当前表的 入住房屋类型 字段 字典表查询方法
			let ruzhudengjiTypes = await this.$api.page('dictionary', {
				page: 1,
				limit: 100,
				dicCode: 'ruzhudengji_types'
			});
			this.ruzhudengjiTypesList = ruzhudengjiTypes.data.list;
		...
		methods: {
			//查询条件切换
			queryChange(e) {
				this.searchForm.ruzhudengjiName = "";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
				...
				if (this.categoryName != 0) {
					params.ruzhudengjiTypes = this.categoryName
				}

				let res = await this.$api.page(`ruzhudengji`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('ruzhudengji', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				if (this.searchForm.ruzhudengjiName) {
					searchForm['ruzhudengjiName'] = this.searchForm.ruzhudengjiName
				}
				if (this.searchForm.ruzhudengjiName) {
					searchForm['ruzhudengjiName'] = this.searchForm.ruzhudengjiName
				}

				let res = await this.$api.page(`ruzhudengji`, searchForm);
				// 如果是第一页数据置空
